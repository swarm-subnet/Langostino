#!/usr/bin/env python3
"""
AI Adapter Simulated Node - Multiple Goals
Deterministic, no-noise, perfect velocity tracking harness for AIFlightNode with multiple sequential goals.
...
- Supports MULTIPLE goal points - drone will navigate to each goal sequentially.
...
class AIAdapterSimulatedMultipleNode(Node):
    def __init__(self):
        super().__init__('ai_adapter_simulated_multiple_node')
        ...
        # Multiple goals as REL ENU - default to 4 goals
        self.declare_parameter('goal_points', [
            4.0, 0.0, 3.0,      # Goal 1: [4.0, 0.0, 3.0]
            4.0, 4.0, 3.0,      # Goal 2: [4.0, 4.0, 3.0]
            0.0, 4.0, 3.0,      # Goal 3: [0.0, 4.0, 3.0]
            0.0, 0.0, 0.0,      # Goal 4: [0.0, 0.0, 0.0]
        ])
        ...
        # Parse multiple goals from flat array
        goal_points_flat = self.get_parameter('goal_points').get_parameter_value().double_array_value
        if len(goal_points_flat) % 3 != 0:
            self.get_logger().error(f"goal_points must be a multiple of 3 (got {len(goal_points_flat)}). Using defaults.")
            goal_points_flat = [4.0, 0.0, 3.0, 4.0, 4.0, 3.0, 0.0, 4.0, 3.0, 0.0, 0.0, 0.0]

        self.goal_points_enu = []
        for i in range(0, len(goal_points_flat), 3):
            self.goal_points_enu.append(np.array(goal_points_flat[i:i+3], dtype=np.float32))

        self.current_goal_index = 0
        self.goal_rel_enu = self.goal_points_enu[self.current_goal_index].copy()
        ...
        # Update current goal geodetic coordinates
        self._update_goal_geodetic()
        ...
        self.goal_reached_flags = [False] * len(self.goal_points_enu)
        self.goal_tolerance = 0.50  # m (success threshold)

        # Track per-goal statistics
        self.goal_start_positions = [self.relative_start_enu.copy()]
        self.goal_distances = []
        self.goal_iterations = []
        self.goal_iteration_start = 0
        ...
        self.get_logger().info(f'🎯 Multiple Goals Mode - {len(self.goal_points_enu)} waypoints configured:')
        for idx, goal in enumerate(self.goal_points_enu):
            marker = '→' if idx == 0 else ' '
            self.get_logger().info(f'   {marker} Goal {idx+1}: [{goal[0]:.2f}, {goal[1]:.2f}, {goal[2]:.2f}]')
        self.get_logger().info('=' * 80)
    ...
    # ────────────────────────────────────────────────────────────────
    # Helper to update goal geodetic from current goal_rel_enu
    # ────────────────────────────────────────────────────────────────
    def _update_goal_geodetic(self):
    ...
    def _physics_tick(self):
        ...
        self.get_logger().info(
            f'🚁 Iter {self.iteration_count:04d} | Goal {self.current_goal_index+1}/{len(self.goal_points_enu)} | '
        ...
        if not self.goal_reached_flags[self.current_goal_index] and distance_to_goal <= self.goal_tolerance:
            self.goal_reached_flags[self.current_goal_index] = True
            self._on_goal_reached(distance_to_goal)

    def _on_goal_reached(self, distance_to_goal: float):
        goal_iters = self.iteration_count - self.goal_iteration_start
        self.goal_iterations.append(goal_iters)

        start_pos = self.goal_start_positions[-1]
        goal_distance = float(np.linalg.norm(self.goal_rel_enu - start_pos))
        self.goal_distances.append(goal_distance)

        self.get_logger().info('=' * 80)
        self.get_logger().info(f'🎯 GOAL {self.current_goal_index+1}/{len(self.goal_points_enu)} REACHED!')
        self.get_logger().info('=' * 80)
        ...
        self.get_logger().info(f'   • Iterations for this goal: {goal_iters}')
        self.get_logger().info('=' * 80)

        # Move to next goal if available
        if self.current_goal_index < len(self.goal_points_enu) - 1:
            self.current_goal_index += 1
            self.goal_rel_enu = self.goal_points_enu[self.current_goal_index].copy()
            self._update_goal_geodetic()
            self.goal_start_positions.append(self.rel_pos_enu.copy())
            self.goal_iteration_start = self.iteration_count

            self.get_logger().info(f'🔄 Moving to Goal {self.current_goal_index+1}/{len(self.goal_points_enu)}: [{self.goal_rel_enu[0]:.2f}, {self.goal_rel_enu[1]:.2f}, {self.goal_rel_enu[2]:.2f}]')
            self.get_logger().info('=' * 80)
        else:
            # All goals reached - print summary and shutdown
            self._on_all_goals_completed()

    def _on_all_goals_completed(self):
        self.get_logger().info('')
        self.get_logger().info('=' * 80)
        self.get_logger().info('🏁 ALL GOALS COMPLETED!')
        self.get_logger().info('=' * 80)
        self.get_logger().info(f'   • Total waypoints reached: {len(self.goal_points_enu)}')
        ...
        self.get_logger().info('')
        self.get_logger().info('Per-Goal Summary:')
        for i in range(len(self.goal_iterations)):
            self.get_logger().info(f'   Goal {i+1}: {self.goal_iterations[i]} iterations, {self.goal_distances[i]:.2f}m distance')

        # Calculate total straight-line distance
        total_straight_line = 0.0
        for i in range(len(self.goal_points_enu)):
            start = self.goal_start_positions[i]
            goal = self.goal_points_enu[i]
            total_straight_line += float(np.linalg.norm(goal - start))

        efficiency = (total_straight_line / self.total_distance_traveled * 100.0) if self.total_distance_traveled > 0 else 0.0
        self.get_logger().info('')
        self.get_logger().info(f'   • Total straight line distance: {total_straight_line:.2f}m')
        self.get_logger().info(f'   • Overall path efficiency: {efficiency:.1f}%')
        self.get_logger().info('=' * 80)
...
def main(args=None):
    ...
    node = AIAdapterSimulatedMultipleNode()
    try:
        rclpy.spin(node)
