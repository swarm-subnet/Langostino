#!/bin/bash
# Network setup script for Swarm system - Ubuntu Server 24.04
...
# Uses wpa_supplicant for WiFi client mode (no NetworkManager)
...
echo "=== [Swarm Setup 24.04] Initial network configuration ==="
...
###############################################
# 1. INSTALL DNSMASQ, HOSTAPD AND WIRELESS TOOLS
###############################################
echo "[Swarm Setup] Installing dnsmasq, hostapd and wireless tools..."
sudo apt update
sudo apt install -y dnsmasq hostapd wireless-tools iw wpasupplicant
...
# Note: wlan0 is NOT configured here - it will be managed by hostapd (AP mode)
# or wpa_supplicant (client mode) via the wifi-manager service
...
# Required for Ubuntu 24.04 / modern kernels
country_code=ES
ieee80211n=1
ieee80211d=1
EOF
...
###############################################
# 9. CREATE WIFI MANAGER (wpa_supplicant based - no NetworkManager)
###############################################
...
check_port_53() {
    if sudo lsof -i:53 2>/dev/null | grep -q "LISTEN"; then
        ...
        # Try to stop systemd-resolved if it's using the port
...
start_ap_services() {
    ...
    while [ $retries -lt $MAX_RETRIES ]; do
        ...
        if sudo systemctl start dnsmasq 2>/dev/null; then
            ...
            echo "[WiFi Manager] Failed to start dnsmasq, retry $((retries+1))/$MAX_RETRIES"
            retries=$((retries+1))
            sleep 2
        fi
    done
    ...
    if ! sudo systemctl start hostapd 2>/dev/null; then
        echo "[WiFi Manager] Failed to start hostapd, checking if masked..."
...
WPAEOF
    ...
    # Remove the commented plaintext password for security
    sudo sed -i '/#psk=/d' $WPA_CONF

    # Start wpa_supplicant
    sudo wpa_supplicant -B -i "$WIFI_INTERFACE" -c $WPA_CONF 2>/dev/null

    # Wait for connection
    echo "[WiFi Manager] Waiting for connection..."
    sleep 5

    # Request DHCP
    sudo dhclient -v "$WIFI_INTERFACE" 2>/dev/null || sudo dhcpcd "$WIFI_INTERFACE" 2>/dev/null || true

    # Additional wait for DHCP
    sleep 3
...
# Function to cleanup wireless interface
cleanup_interface() {
    echo "[WiFi Manager] Cleaning up interface..."
...
# Try to connect to known networks using wpa_supplicant
echo "[WiFi Manager] Scanning for known networks..."
...
if [ -f "$CONFIG_FILE" ]; then
    ...
    # Scan for networks
    echo "[WiFi Manager] Performing WiFi scan..."
    SCAN_RESULT=$(sudo iw "$WIFI_INTERFACE" scan 2>/dev/null | grep "SSID:" | sed 's/.*SSID: //' | sort | uniq)
        ...
        while IFS=: read -r SSID PASSWORD; do
            ...
            if echo "$SCAN_RESULT" | grep -qF "$SSID"; then
                ...
                if connect_with_wpa "$SSID" "$PASSWORD"; then
                    CONNECTED=true
                    IP_ADDR=$(ip addr show "$WIFI_INTERFACE" | grep "inet " | awk '{print $2}')
                    echo "[WiFi Manager] Connected to $SSID with IP $IP_ADDR"
                    break
                else
                    echo "[WiFi Manager] Failed to connect to $SSID, trying next..."
                    cleanup_interface
                fi
            fi
        done < "$CONFIG_FILE"
    else
        echo "[WiFi Manager] No networks detected in scan"
    fi
fi
...
if ! grep -q "^$SSID:" "$CONFIG_FILE" 2>/dev/null; then
    ...
    echo "Network '$SSID' already in configuration"
fi
...
[Unit]
Description=WiFi Auto Manager (client or AP) - Ubuntu 24.04
